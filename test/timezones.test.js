...
describe('Timezones', () => {
  ...
  describe('Timezones by country', () => {
    ...
    it('should handle lowercase country codes', () => {
      expect(timezones.getTimezonesByCountry('us')).to.deep.equal(
        timezones.getTimezonesByCountry('US')
      );
    });
  ...
  describe('Countries by timezone', () => {
    it('should find countries that use a timezone', () => {
      expect(timezones.getCountriesForTimezone('Europe/London')).to.include(
        'GB'
      );
      expect(timezones.getCountriesForTimezone('America/New_York')).to.include(
        'US'
      );
    });

    it('should return empty array for invalid timezones', () => {
      expect(timezones.getCountriesForTimezone('Not/Real')).to.be.an('array')
        .that.is.empty;
      expect(timezones.getCountriesForTimezone('')).to.be.an('array').that.is
        .empty;
    });
  });
